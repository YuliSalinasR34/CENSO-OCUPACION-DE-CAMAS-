...
async function initSchema() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS referencias_ips (
      id SERIAL PRIMARY KEY,
      fecha DATE NOT NULL,
      ips_cod TEXT NOT NULL REFERENCES ips(cod),
      pacientes_presentados INTEGER NOT NULL DEFAULT 0,
      pacientes_aceptados INTEGER NOT NULL DEFAULT 0,
      pct_aceptacion NUMERIC NOT NULL DEFAULT 0,
      observaciones TEXT,
      usuario_id INTEGER NOT NULL REFERENCES users(id),
      creado_en TIMESTAMPTZ NOT NULL DEFAULT now(),
      actualizado_en TIMESTAMPTZ NOT NULL DEFAULT now(),
      UNIQUE(fecha, ips_cod)
    );
    CREATE INDEX IF NOT EXISTS idx_referencias_fecha ON referencias_ips(fecha);
    CREATE INDEX IF NOT EXISTS idx_referencias_ips ON referencias_ips(ips_cod);
  `);
}
