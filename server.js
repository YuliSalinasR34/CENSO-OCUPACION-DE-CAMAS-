...
app.get("/api/dashboard/resumen", authMiddleware, h(async (req, res) => {
  ...
  // Cuántos días distintos con datos hay en el rango — para promediar, no sumar sin sentido.
  const diasConDatos = (await pool.query(
    `SELECT COUNT(DISTINCT c.fecha)::int AS n FROM censos c JOIN ips i ON i.cod = c.ips_cod WHERE ${baseWhere}`,
    params
  )).rows[0].n || 1;
  ...
  // "Camas" es una foto (stock), no algo que se acumula día a día: promediamos entre los días
  // con datos para que el número sea legible (ej. "camas habilitadas: 2050", no "615.000").
  const habilitadas = Math.round((Number(kpis.habilitadas || 0) / diasConDatos) * 10) / 10;
  const ocupadas = Math.round((Number(kpis.ocupadas || 0) / diasConDatos) * 10) / 10;
  const disponiblesProm = Math.round((Number(kpis.disponibles || 0) / diasConDatos) * 10) / 10;
  const ocupacionFamisanar = Math.round((Number(kpis.ocupacion_famisanar || 0) / diasConDatos) * 10) / 10;
  ...
  // Serie mensual: toda la historia disponible (no solo el período elegido), respetando
  // los filtros geográficos, para ver la tendencia de mes a mes cuando el período es "Mes".
  const filtrosSoloGeo = [];
  const paramsGeo = [];
  const addFiltroGeo = (clause, val) => { paramsGeo.push(val); filtrosSoloGeo.push(`${clause} $${paramsGeo.length}`); };
  if (regional) addFiltroGeo("i.regional =", regional);
  if (zonal) addFiltroGeo("i.zonal =", zonal);
  if (municipio) addFiltroGeo("i.municipio =", municipio);
  if (ipsCod) addFiltroGeo("i.cod =", ipsCod);
  const whereGeo = filtrosSoloGeo.length ? "WHERE " + filtrosSoloGeo.join(" AND ") : "";
  const serieTiempoMensual = (await pool.query(
    `SELECT to_char(c.fecha, 'YYYY-MM') AS mes,
            COUNT(DISTINCT c.fecha)::int AS dias,
            COALESCE(SUM(c.camas_habilitadas),0) AS habilitadas,
            COALESCE(SUM(c.ocupacion_ips),0) AS ocupadas,
            COALESCE(SUM(c.ocupacion_famisanar),0) AS famisanar
     FROM censos c JOIN ips i ON i.cod = c.ips_cod
  ...
  res.json({
    ...
    serieTiempoMensual: serieTiempoMensual.map(r => ({ mes: r.mes, ...pct(r) })),
