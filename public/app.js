...
async function importarHistoricoPorMes() {
  ...
  try {
    ...
    // Agrupamos por MES (columna "fecha de diligenciamineto") — menos partes, cada una más
    // grande pero el servidor ya procesa por lotes internamente, así que sigue siendo liviano.
    const COL_FECHA = 40;
    ...
    for (const fila of datos) {
      ...
      if (!mesKey) mesKey = "sin-fecha";
      if (!grupos.has(mesKey)) grupos.set(mesKey, []);
      grupos.get(mesKey).push(fila);
    }

    // Dentro de cada mes, si trae demasiadas filas, lo partimos otra vez en trozos más chicos
    // para que ninguna solicitud individual sea pesada.
    const TAMANO_MAX_POR_PARTE = 600;
    const partes = []; // { etiqueta, filas }
    for (const [mes, filasMes] of [...grupos.entries()].sort()) {
      if (filasMes.length <= TAMANO_MAX_POR_PARTE) {
        partes.push({ etiqueta: mes, filas: filasMes });
      } else {
        const numSubpartes = Math.ceil(filasMes.length / TAMANO_MAX_POR_PARTE);
        for (let i = 0; i < numSubpartes; i++) {
          partes.push({ etiqueta: `${mes} (${i + 1}/${numSubpartes})`, filas: filasMes.slice(i * TAMANO_MAX_POR_PARTE, (i + 1) * TAMANO_MAX_POR_PARTE) });
        }
      }
    }
    ...
    for (let i = 0; i < partes.length; i++) {
      ...
      const etiqueta = `Parte ${i + 1} de ${partes.length} (${parte.etiqueta})`;
      progreso.innerHTML = `<div class="ok-msg">${etiqueta}… no cierres esta pantalla.</div>`;
      const data = await subirLoteConReintentos(header, parte.filas, etiqueta, progreso);
    ...
    progreso.innerHTML = `<div class="ok-msg">✅ Carga histórica completa: ${partes.length} partes procesadas (${grupos.size} meses). ${totales.censosInsertados} registros de censo, ${totales.referenciasInsertadas} registros de referencias, ${totales.filasOmitidas} filas omitidas.</div>`;
  } catch (err) {
