  ...
  main.innerHTML = `
    ...
    <div class="card" id="card-historico">
      ...
      <p class="desc" style="margin-bottom:14px;">
        ...
        que traiga el archivo. Se puede repetir sin duplicar nada. Para archivos grandes, el sistema los divide por mes
...
async function renderDashboard(main) {
  ...
  const etiquetaMes = (m) => { const [y, mm] = m.split("-"); return `${nombresMes[Number(mm) - 1]} ${y}`; };
...
function pintarDashboard(box, data) {
    ...
    return `
      <svg viewBox="0 0 ${w} ${h}" style="width:100%;height:190px;">
        ...
        ${serie.map((s, idx) => {
          ...
          return `
            <circle cx="${xi}" cy="${yi}" r="3.5" fill="#2E9E6D"><title>${s.fecha} · IPS: ${s.pctIps}%</title></circle>
            <circle cx="${xf}" cy="${yf}" r="3.5" fill="#0F3460"><title>${s.fecha} · Famisanar: ${s.pctFamisanar}%</title></circle>
            <text x="${xi}" y="${h - 4}" font-size="9" fill="#5B6472" text-anchor="middle">${s.fecha.slice(5)}</text>
          `;
        }).join("")}
      </svg>
    `;
  ...
  box.innerHTML = `
    ...
    <div class="grid-2">
      ...
      <div class="card">
        <h3>IPS faltantes hoy, por regional</h3>
        ${pieChart()}
      </div>
    </div>
...
async function importarHistoricoPorMes() {
  ...
  try {
    ...
    const COL_FECHA = 40; // "fecha de diligenciamineto"
    const grupos = new Map(); // "2026-01" -> [filas]
    let sinFecha = 0;
    for (const fila of datos) {
      const v = fila[COL_FECHA];
      let mesKey = null;
      if (v instanceof Date) {
        mesKey = `${v.getFullYear()}-${String(v.getMonth() + 1).padStart(2, "0")}`;
      } else if (typeof v === "number") {
        const ms = Math.round((v - 25569) * 86400 * 1000);
        const d = new Date(ms);
        if (!isNaN(d.getTime())) mesKey = `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, "0")}`;
      } else if (typeof v === "string") {
        const m = v.match(/^(\d{4})-(\d{2})/);
        if (m) mesKey = `${m[1]}-${m[2]}`;
      }
      if (!mesKey) { sinFecha++; continue; }
      if (!grupos.has(mesKey)) grupos.set(mesKey, []);
      grupos.get(mesKey).push(fila);
    }

    const meses = [...grupos.keys()].sort();
    if (meses.length === 0) {
      progreso.innerHTML = `<div class="error-msg">No se encontró ninguna fecha válida en la columna "fecha de diligenciamineto".</div>`;
      btn.disabled = false;
      return;
    }

    let totales = { ipsAseguradas: 0, censosInsertados: 0, referenciasInsertadas: 0, filasOmitidas: sinFecha };
    for (let i = 0; i < meses.length; i++) {
      const mes = meses[i];
      progreso.innerHTML = `<div class="ok-msg">Subiendo mes ${i + 1} de ${meses.length} (${mes})… no cierres esta pantalla.</div>`;

      const wsMes = XLSX.utils.aoa_to_sheet([header, ...grupos.get(mes)]);
      const wbMes = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wbMes, wsMes, "ocupacion");
      const arrayBuf = XLSX.write(wbMes, { type: "array", bookType: "xlsx" });
      const blob = new Blob([arrayBuf], { type: "application/octet-stream" });

      const formData = new FormData();
      formData.append("file", blob, `historico-${mes}.xlsx`);

      const headers = {};
      if (state.token) headers.Authorization = "Bearer " + state.token;
      const res = await fetch("/api/admin/importar-historico", { method: "POST", headers, body: formData });
      let data;
      try { data = await res.json(); } catch (e) { throw new Error(`El servidor no respondió correctamente al subir el mes ${mes} (posible caída temporal). Intenta de nuevo con ese mes solo.`); }
      if (!res.ok) throw new Error(`Mes ${mes}: ${data.error || "error desconocido"}`);
    ...
    progreso.innerHTML = `<div class="ok-msg">✅ Carga histórica completa: ${meses.length} meses procesados (${meses.join(", ")}). ${totales.censosInsertados} registros de censo, ${totales.referenciasInsertadas} registros de referencias, ${totales.filasOmitidas} filas omitidas.</div>`;
  } catch (err) {
    progreso.innerHTML = `<div class="error-msg">${err.message}</div>`;
  } finally {
    btn.disabled = false;
  }
}
