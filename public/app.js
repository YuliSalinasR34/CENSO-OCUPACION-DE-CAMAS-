...
async function renderAdminIps(main) {
  ...
  main.innerHTML = `
    ...
    ${state.permisos.puedeAdministrarUsuarios ? `
    <div class="card">
      ...
      <p class="desc" style="margin-bottom:14px;">
        ...
        que traiga el archivo. Se puede repetir sin duplicar nada — actualiza si ya existía ese día para esa IPS.
        Archivos grandes (miles de filas) pueden tardar un minuto en procesarse.
      </p>
  ...
  const btnHistorico = document.getElementById("btn-importar-historico");
  if (btnHistorico) {
    btnHistorico.addEventListener("click", async () => {
      ...
      btnHistorico.textContent = "Procesando… puede tardar hasta un minuto";
      msg.innerHTML = `<div class="ok-msg">Importando histórico, no cierres esta pantalla…</div>`;
      try {
        const headers = {};
        if (state.token) headers.Authorization = "Bearer " + state.token;
        ...
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "Error al importar.");
        msg.innerHTML = `<div class="ok-msg">Listo: ${data.ipsAseguradas} IPS aseguradas, ${data.censosInsertados} registros de censo creados/actualizados, ${data.referenciasInsertadas} registros de referencias, ${data.filasOmitidas} filas omitidas.</div>`;
        renderAdminIps(main);
      } catch (err) {
        msg.innerHTML = `<div class="error-msg">${err.message}</div>`;
        btnHistorico.disabled = false;
        btnHistorico.textContent = "Importar histórico completo";
      }
    });
  }
